...
`Base Pitchtt{{first known use: Sumianvoice (but probably not the first use)\n\nNote it was used in Scinguistics in 2020 and probably elsewhere but defined as the fundamental freq rather than a speech pattern feature like this is defined as. It was much later (2022) defined as a return to note and tested in a research project conducted on the Voice Art Project Discord server, but used before then for some time as an automatically returned-to pitch to aid in changing pitch and keeping that change consistent.}} is the pitch a voice naturally returns to after intonation. It is determined by t[[closedquotient|CQ]] and t[[vfvmpressure|pressure]] and represents the pitch that a voice is naturalised to. It is usually the lowest note a voice will go to in any configuration, and is sort of like an anchor or "home position". For example if we speak at F3, the voice will go above F3 for intonation and then eventually return to F3 as the lowest note. It can be changed at will once trained, and is the basis of t[[naturalisation|pitch naturalisation]]. It is possible for a voice to hit lower notes than the base pitch it is naturalised to, but generally this will be rare or used for expressiveness of speech rather than a natural phenomenon.
